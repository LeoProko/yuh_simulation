...
Bot::Bot(const Bot* mother, const Bot* father)
    ...
    militancy_ = (mother->militancy_ + father->militancy_) / 2 + Rand::random_() % mutation - mutation / 2;
    attractiveness_ = (mother->attractiveness_ + father->attractiveness_) / 2 + Rand::random_() % mutation - mutation / 2;
    intelligence_ = (mother->intelligence_ + father->intelligence_) / 2 + Rand::random_() % mutation - mutation ;
    children_amount_ = (mother->children_amount_ + father->children_amount_) / 2 + Rand::random_() % mutation - mutation  / 2;
    children_health_ = (mother->children_health_ + father->children_health_) / 2 + Rand::random_() % mutation - mutation / 2;
    calibrate();
}
