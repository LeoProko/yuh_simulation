...
Перейдите в директорию `visualization`, там будет лежать файл `results.json`
со всеми необходимыми данными для визуализации и исполняемые файлы
`make_charts.sh` и `make_vieo.sh`. Запустите их соответственно по очереди.
На выходи вы получите видео с симуляцией.
